...
float maxRoll = 100.0;     //Max roll angle in degrees for angle mode (maximum ~70 degrees), deg/sec for rate mode 
float maxPitch = 100.0;    //Max pitch angle in degrees for angle mode (maximum ~70 degrees), deg/sec for rate mode
...
float Ki_roll_rate = 1.2;     //Roll I-gain - rate mode
...
float Ki_pitch_rate = 0.8;    //Pitch I-gain - rate mode
float Kd_pitch_rate = 0.0; //Pitch D-gain - rate mode (be careful when increasing too high, motors will begin to overheat!)

float Kp_yaw = 0.2;           //Yaw P-gain
float Ki_yaw = 0.8;          //Yaw I-gain
float Kd_yaw = 0.0;       //Yaw D-gain (be careful when increasing too high, motors will begin to overheat!)

// Actuator locations (from CG)
float r_act1_m[3] = {0.156475, 0, 0.071189}; // position of 1st tiltrotor
float r_act2_m[3] = {-0.156475, 0, 0.071189}; // position of 2nd tiltrotor

// Max motor force
float throttle2motorForce = 6.6; // convert throttle (0-1) to motor force(newtons)

// Actuator states
float delta = 0.0; // +- mixing force to the motors
float t1 = 0.0; // angle of 1st tiltrotor (radians)
float t2 = 0.0; // angle of 2nd tiltrotor (radians)

// Max allowable moments
float max_roll_moment_nm = 0.5;
float max_pitch_moment_nm = 0.5;
float max_yaw_moment_nm = 0.5;
...
//Radio:
//Note: If using SBUS, connect to pin 7 (RX2), if using DSM, connect to pin 15 (RX3)
...
const int m2Pin = 6;
const int m3Pin = 5;
const int m4Pin = 1; // Connected to nothing on ctrl v2 fc
const int m5Pin = 2; // Connected to nothing on ctrl v2 fc
const int m6Pin = 3; // Connected to nothing on ctrl v2 fc
//PWM servo or ESC outputs:
const int servo1Pin = 8;
const int servo2Pin = 9;
const int servo3Pin = 10;
const int servo4Pin = 11; // Connected to nothing on ctrl v2 fc
const int servo5Pin = 12; // Connected to nothing on ctrl v2 fc
const int servo6Pin = 14; // Connected to nothing on ctrl v2 fc
const int servo7Pin = 15; // Connected to nothing on ctrl v2 fc
...
//Actuator parameters
float m1_throttle, m2_throttle, s1_angle, s2_angle = 0;
...
void setup() {
  ...
  servo1.attach(servo1Pin, 400, 2600); //Pin, min PWM value, max PWM value
  servo2.attach(servo2Pin, 400, 2600);
  servo3.attach(servo3Pin, 400, 2600);
  servo4.attach(servo4Pin, 400, 2600);
  servo5.attach(servo5Pin, 400, 2600);
  servo6.attach(servo6Pin, 400, 2600);
  servo7.attach(servo7Pin, 400, 2600);
  ...
  //Arm servo channels
  servo1.write(45); //Command servo angle from 0-180 degrees (500 to 2500 PWM)
  servo2.write(45); //Set these to 90 for servos if you do not want them to briefly max out on startup
  servo3.write(45); //Keep these at 0 if you are using servo outputs for motors
  servo4.write(0);
  servo5.write(0);
  servo6.write(0);
  servo7.write(0);
...
void allocation() {
  //DESCRIPTION: Computes servo angles and throttles based on PIDs.

  //Obtain current desired moments & current throttle force
  float Mx = roll_PID * max_roll_moment_nm; // desired axial moments
  float My = pitch_PID * max_pitch_moment_nm; 
  float Mz = yaw_PID * max_yaw_moment_nm;
  float f = thro_des * 2 * throttle2motorForce; // commanded motor force.

  //Compute angles and delta force.
  if(thro_des < 0.05){
    //Set all to 0 if the throttle is too low.
    t1 = 0.0;
    t2 = 0.0;
    delta = 0.0;
  }
    ...
    //Compute t1
    float numerator_t1 = (Mx * r_act2_m[0] + Mz * r_act2_m[2]) * (r_act1_m[0] - r_act2_m[0]);
    float denominator_t1 = (My + 2 * f * r_act2_m[0]) * (r_act1_m[0] * r_act2_m[2] - r_act2_m[0] * r_act1_m[2]);
    t1 = numerator_t1 / denominator_t1;
    
    //Compute t2
    float numerator_t2 = (Mx * r_act1_m[0] + Mz * r_act1_m[2]) * (r_act1_m[0] - r_act2_m[0]);
    float denominator_t2 = (My + 2 * f * r_act1_m[0]) * (r_act1_m[0] * r_act2_m[2] - r_act2_m[0] * r_act1_m[2]);
    t2 = numerator_t2 / denominator_t2;
    
    //Compute delta
    delta = (My + f * r_act1_m[0] + f * r_act2_m[0]) / (r_act1_m[0] - r_act2_m[0]);
...
void controlMixer() {
  ...
  //Get actuator angles based on current PID values and scale them
  allocation();
  float scaled_delta = delta * 1.0/throttle2motorForce;
  float scaled_t1 = (t1 * 180.0/PI + 90.0)/180.0;
  float scaled_t2 = (-t2 * 180.0/PI + 90.0)/180.0;
//
//    Serial.print("t1:");
//    Serial.print(scaled_t1);
//    Serial.print("t2:");
//    Serial.print(scaled_t2);
//    Serial.print("delta");
//    Serial.println(scaled_delta);
  ...
  s1_command_scaled = scaled_t1; //Front
  s2_command_scaled = scaled_t2; //Back
  s3_command_scaled = 0;
  s4_command_scaled = 0;
  s5_command_scaled = 0;
  s6_command_scaled = 0;
...
void scaleCommands() {
  ...
  s1_command_PWM = s1_command_scaled*180 + s1_trim;
  s2_command_PWM = s2_command_scaled*180 + s2_trim;
...
void getCommands() {
   ...
   */
    ...
    if (sbus.read(&sbusChannels[0], &sbusFailSafe, &sbusLostFrame))
    {
      ...
      // flip channel if needed
      channel_4_pwm = flip(channel_4_pwm);
    }
...
int flip(int channel_pwm) {
  return -1 * channel_pwm + 3000;
}
